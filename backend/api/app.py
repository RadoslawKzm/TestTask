...
async def lifespan(func_app: FastAPI) -> typing.AsyncContextManager[None]:
    ...
    Instrumentator().instrument(func_app).expose(func_app, endpoint="/metrics")
    yield
...
@app.middleware("http")
async def log_user_metadata(request: Request, call_next: typing.Any) -> typing.Any:
...
@app.middleware("http")
async def exception_logger(request: Request, call_next: typing.Any) -> typing.Any:
    response = await call_next(request)
    log_msg: str = f"{request.method} {request.url.path} {response.status_code}"
    metadata_str = log_msg
    logger.opt(lazy=True).info(log_msg)
    if response.status_code < 400:
        logger.opt(lazy=True).debug(metadata_str)
    elif response.status_code == 422:
        logger.opt(lazy=True).log(log_config.request_validation_exception, metadata_str)
    elif 400 <= response.status_code < 500:
        logger.opt(lazy=True).log(log_config.http_exception, metadata_str)
    elif response.status_code == 501:
        logger.opt(lazy=True).exception(log_config.unexpected_exception, metadata_str)
    else:
        logger.opt(lazy=True).exception(log_config.handled_internal_exception, metadata_str)
    return response
...
def init_listeners(func_app: FastAPI) -> FastAPI:
    ...
    async def custom_generic_exception_handler(request: Request, exc: Exception) -> Response:
        ...
        return JSONResponse(
            ...
            content={"detail": "Internal Server Error. Our team is already working on it."},
            headers=headers,
        )
